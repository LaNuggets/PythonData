...
df['Age'] = df['Age'].fillna(df['Age'].mean()) # remplir les ages manquant par l'age moyen
...
model = LogisticRegression(max_iter=1000) # création d'une instance de LogisticRegression
# model = RandomForestClassifier(n_estimators=100, random_state=42)
model.fit(X_train, y_train) # entrainement du model
y_pred = model.predict(X_test) # résultat des prédictions
